...
test('compares two clips in the modal, switching via dropdown and nudging offset', async () => {
  await page.getByTestId('compare-start').click()

  // Modal opens with both panes defaulting to the two most recent clips.
  ...
  // The dropdown switches pane A's clip (two options exist).
  const optionCount = await page.getByTestId('compare-select-a').locator('option').count()
  expect(optionCount).toBe(2)
  await page.getByTestId('compare-select-a').selectOption({ index: 0 })
  await expect.poll(async () => (await videoState('a')).w).toBe(1280)

  // Pause, then an offset nudge shifts B ahead of A.
  await page.getByTestId('compare-playpause').click()
